...
// --- Helper: parse CSV in the Catalog textarea (shared by preview & save) ---
...
// -------------------- State --------------------
    ...
    // ---- Discrete Sections Catalog (up to 10 rows) ----
    // Columns: name, I, St, Sb, and optional A (area) & wd (weight density)
    let sectionsCatalog = [
    ...
    function buildBeam() {
      ...
      // Always (re)initialize segment lengths to evenly space the total length on Build
      ...
      // Rebuild nodes to match new segment count
    ...
    function drawBeam() {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        // Darker gray shading (about 60%→30% lightness from light to heavy)
        const wmag = Math.abs((seg.wd||0)*(seg.A||0));
        const tGray = maxAbsW > 0 ? Math.min(1, wmag / maxAbsW) : 0; // 0..1
        const light = Math.round(60 - tGray * 30); // 60% → 30%
    ...
    function plotShearFromMoment(){
      ...
      const ctx = document.getElementById('shearPlot').getContext('2d'); if (shearChart) destroy();
    ...
    // ------------- Discrete Catalog + Optimizer (unchanged logic) -------------
          ...
          Swal.fire({
            ...
            html: `
              <div style="display:grid; grid-template-columns:1fr; gap:10px; width:min(680px, 90vw);">
                <div>
                  <canvas id="chartS" width="640" height="240"></canvas>
                </div>
                <div>
                  <canvas id="chartI" width="640" height="240"></canvas>
                </div>
              </div>
            `,
            didOpen: ()=>{
      ...
      Swal.fire({
        ...
        html: `
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <label><span>σ_allow (absolute)</span><input id="sigAllowD" class="swal2-input" value="20"></label>
